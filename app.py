...
@st.cache_data
def load_ec2():
    return pd.read_csv(ec2_path, parse_dates=["CreationDate"], low_memory=False)
...
@st.cache_data
def load_s3():
    return pd.read_csv(s3_path, parse_dates=["CreationDate"])

# Actually load the data (cached)
df_ec2 = load_ec2()
df_s3  = load_s3()
...
underutilized_ec2['SavingsPerHour'] = underutilized_ec2['CostUSD'] * 0.5
underutilized_ec2 = underutilized_ec2.sort_values('SavingsPerHour', ascending=False).head(10)

# 2. Expensive S3 STANDARD buckets
expensive_s3 = filtered_s3[
    (filtered_s3['StorageClass'] == 'STANDARD') &
    (filtered_s3['TotalSizeGB'] > 100)
].copy()
expensive_s3['MonthlySavings'] = expensive_s3['TotalSizeGB'] * 0.01
expensive_s3 = expensive_s3.sort_values('MonthlySavings', ascending=False).head(10)
...
with tab2:
    ...
    with col1:
        st.write("**CPU Utilization Distribution**")
        fig_cpu = px.histogram(filtered_ec2, x="CPUUtilization", nbins=30,
                               title="EC2 CPU Usage (%)", color="Region")
        st.plotly_chart(fig_cpu, use_container_width=True)
    ...
    st.write("**Average Cost per Region**")
    fig_bar = px.bar(avg_cost_region, x="Region", y="CostUSD",
                     title="Avg Cost by Region", text="CostUSD")
    fig_bar.update_traces(texttemplate='%{text:.3f}')
    st.plotly_chart(fig_bar, use_container_width=True)
...
with tab3:
    ...
    with col1:
        st.write("**Total Storage by Region**")
        fig_storage = px.bar(total_storage_region, x="Region", y="TotalSizeGB",
                             title="Total S3 Storage (GB)", text="TotalSizeGB")
        fig_storage.update_traces(texttemplate='%{text:.1f}')
        st.plotly_chart(fig_storage, use_container_width=True)
...
with tab5:
    ...
    if not underutilized_ec2.empty:
        ...
        st.write("**Top 10 underutilized instances (CPU < 20%)**")
        st.dataframe(
            underutilized_ec2[['ResourceId', 'InstanceType', 'Region',
                               'CPUUtilization', 'CostUSD', 'SavingsPerHour']].round(2),
            use_container_width=True
        )
        st.info("**Action**: Downsize `m5.large` → `t3.medium`, or use Spot Instances.")
    else:
    ...
    if not expensive_s3.empty:
        ...
        st.write("**Top 10 large STANDARD buckets (>100 GB)**")
        st.dataframe(
            expensive_s3[['BucketName', 'Region', 'TotalSizeGB',
                          'CostUSD', 'MonthlySavings']].round(2),
            use_container_width=True
        )
        st.info("**Action**: Move to `STANDARD_IA` or `GLACIER` → Save ~50% on storage.")
    else:
...
st.sidebar.download_button(
    "Download Filtered EC2 Data",
    filtered_ec2.to_csv(index=False).encode(),
    "filtered_ec2.csv",
    "text/csv"
)
st.sidebar.download_button(
    "Download Filtered S3 Data",
    filtered_s3.to_csv(index=False).encode(),
    "filtered_s3.csv",
    "text/csv"
)
